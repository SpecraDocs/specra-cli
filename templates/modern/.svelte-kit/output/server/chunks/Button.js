import { g as sanitize_props, i as spread_props, j as slot, a as attr, d as attr_class, f as clsx, k as derived, l as store_get, m as unsubscribe_stores, s as stringify, e as escape_html, t as clsx$1, q as attributes } from "./root.js";
import { I as Icon, T as Triangle_alert } from "./triangle-alert.js";
import { t as themeStore } from "./tabs.js";
...
function Logo($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    let { logo, alt = "Logo", className = "h-8 w-8 object-contain" } = $$props;
    let currentSrc = derived(() => {
      if (!logo) return "";
      if (typeof logo === "string") return logo;
      return store_get($$store_subs ??= {}, "$themeStore", themeStore) === "dark" ? logo.dark : logo.light;
    });
    if (logo) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<img${attr("src", currentSrc())}${attr("alt", alt)}${attr_class(clsx(className))}/>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]-->`);
...
function SiteBanner($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { config } = $$props;
    ...
    if (isEnabled() && true) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div${attr_class(`relative flex items-center justify-center gap-2 px-4 py-2.5 text-sm border-b ${stringify(typeStyles[bannerType()] || typeStyles.info)}`)} role="banner"><span${attr_class(`shrink-0 ${stringify(iconColor[bannerType()] || iconColor.info)}`)}>`);
      if (bannerType() === "info") {
        $$renderer2.push("<!--[0-->");
        Info($$renderer2, { class: "h-4 w-4" });
      ...
      $$renderer2.push(`<!--]--></span> <span class="text-center font-medium">${escape_html(banner()?.message)}</span> `);
      if (isDismissible()) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<button class="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-md hover:bg-black/10 dark:hover:bg-white/10 transition-colors" aria-label="Dismiss banner">`);
...
const buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive [&>a]:inline-flex [&>a]:items-center [&>a]:gap-2 [&>a]:whitespace-nowrap", {
  variants: {
    variant: {
